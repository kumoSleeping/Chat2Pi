...
export function migrateHome(home: string) {
  ...
  if (!bindings.length && !logins.length && !existsSync(journalPath)) return;
  ...
  const destinations = new Map<string, { value: any; device: boolean }>();
  const source = (path: string) => {
    const text = readPrivate(path);
    sources.set(path, text);
    return JSON.parse(text);
  };
  const plan = (path: string, value: any, device: boolean) => {
  ...
  const finishJournal = () => {
    const journal = JSON.parse(readPrivate(journalPath)) as {
      sources: { path: string; digest: string }[];
      targets: { path: string; digest: string }[];
    };
    for (const entry of journal.targets) {
      if (hash(readPrivate(entry.path)) !== entry.digest)
        throw Error(
          "Migrated configuration changed; remaining originals preserved",
        );
    }
    for (const entry of journal.sources) {
      if (
        ...
        throw Error(
          "Legacy configuration changed; remaining originals preserved",
        );
    }
    for (const entry of journal.sources)
  ...
  try {
    ...
    for (const [path, { value, device }] of destinations) {
      ...
      if (device) loadAgents(path);
      else loginSchema.parse(JSON.parse(readPrivate(path)));
    }
    for (const [path, original] of sources) {
      if (readPrivate(path) !== original)
        throw Error(
          "Legacy configuration changed during migration; originals preserved",
        );
    }
    if (sources.size) {
    ...
    if (sources.size)
      console.log(
        `Configuration migrated: ${destinations.size} account/device file(s); ${sources.size} replaced legacy file(s) removed.`,
      );
  } finally {
    closeSync(lock);
    unlinkSync(lockPath);
