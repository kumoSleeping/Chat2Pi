...
function control(s: State, stop = false): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(2000, () =>
      req.destroy(Error("Local service not responding")),
    );
    req.on("error", reject);
    req.end();
  });
}
function configs(home: string) {
  ...
  for (const c of result) {
    ...
    if (ids.has(id)) throw Error("Duplicate device binding");
    ids.add(id);
  }
  return result;
}
export async function serve(home: string) {
  ...
  const clean = () => {
    try {
      if (state(home)?.id === id) unlinkSync(statePath(home));
    } catch {}
  };
  const stop = () => {
    if (stopping) return;
    ...
    log(
      "INFO",
      "Stopping Chat2Pi; cancelling active tools and disconnecting devices…",
    );
  ...
  const server = createServer((req, res) => {
    ...
    if (isStop) res.once("finish", stop);
  });
  try {
    for (const config of list) agents.push(startAgent(config));
...
async function backgroundCommand(
  ...
  if (!["start", "stop", "restart", "status"].includes(command)) return false;
  if (command === "status") {
    const s = state(home);
    let current: any;
    if (s) {
      try {
        ...
        if (alive(s.pid))
          throw Error(
            "Service process exists but control is unavailable; inspect runtime/service.log",
          );
      }
    }
    console.log(
  ...
  try {
    ...
    if (existsSync(pendingPath)) {
      ...
      if (
        ...
        throw Error(
          "Service is still starting; inspect runtime/service.log before retrying",
        );
    ...
    if (s) {
      try {
        ...
        if (alive(s.pid))
          throw Error(
            "Service process exists but control is unavailable; refusing to start a duplicate",
          );
    ...
    if ((command === "stop" || command === "restart") && s) {
      ...
      for (let i = 0; i < 100 && state(home)?.id === s.id; i++)
        await sleep(100);
      if (state(home)?.id === s.id) throw Error("Service is still stopping");
      s = undefined;
    }
    if (command === "stop") {
    ...
    if (s) {
      if (session)
        throw Error(
          "Chat2Pi is already running; use stop before starting a foreground session",
        );
    ...
    if (child.pid) savePrivate(pendingPath, JSON.stringify({ pid: child.pid }));
    ...
    for (let i = 0; i < 300; i++) {
      ...
      if (started && started.pid === child.pid) {
        ...
        console.log(
          "Chat2Pi started; use status for local state and list_devices for online state",
        );
        return true;
      }
...
export async function serviceCommand(
  ...
  if (existsSync(logPath)) offset = statSync(logPath).size;
  ...
  const poll = () => {
    ...
    try {
      ...
      offset += count;
      if (count) display(decoder.write(buffer.subarray(0, count)));
    } finally {
      closeSync(fd);
    }
  };
  ...
  console.log(
    "Foreground session — Ctrl+C stops the service and disconnects devices.",
  );
  try {
    ...
    while (!abort.signal.aborted) {
      ...
      if (!current || current.pid !== child?.pid) break;
    ...
    try {
      if (child) {
        ...
        if (child.exitCode === null && child.signalCode === null) {
          ...
          if (child.exitCode === null && child.signalCode === null)
            throw Error(
              "Service has not stopped; inspect status before restarting",
            );
        }
        if (
          current &&
