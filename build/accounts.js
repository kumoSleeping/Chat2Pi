...
export async function accountCommand(command, o) {
    ...
    if (command === "login-import") {
        ...
        if (raw.binding || raw.device_key)
            throw Error("This is a device file; use device-import on the target computer");
    ...
    if (command === "device-import") {
        ...
        const raw = JSON.parse(readPrivate(resolve(o.bundle)));
        if (raw.login_key)
            throw Error("This is an account file for ChatGPT authorization; use a device file on this computer");
        ...
        const outputs = o.config || o.credentials
            ? [
                [cp, local],
                [path, binding],
            ]
        ...
        for (const [file, value] of outputs)
            if (!existsSync(file))
                savePrivate(file, json(value));
        console.log(`Device: ${binding.device_id}\nWorkspace: ${workspace}\nAccess: ${access}\nTools: ${tools.join(", ")}`);
        if (o.start) {
            const { serviceCommand } = await import("./service.js");
            await serviceCommand(o.background ? "restart" : "start", home, o.background);
        }
        return true;
    }
    ...
    if (command === "claim") {
        ...
        console.log(`${device ? "Device configuration" : "ChatGPT plugin account credentials"} saved privately: ${output}`);
        return true;
    }
    if (command === "call") {
