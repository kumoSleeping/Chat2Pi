...
import { requireSecureUrl, secureEqual, toolNames, } from "./config.js";
const hello = z
    .object({
...
export async function startGateway(config) {
    ...
    const auth = new PersonalAuth(origin, config.oauth_state_file, config.owner_key_file);
    auth.mount(app);
    ...
    app.post("/mcp", bearer, express.json({ limit: "1mb" }), async (req, res) => {
        ...
        server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: "list_devices",
                    description: "列出自己的电脑代号、在线状态和允许的工具；操作电脑前先查询。",
                    inputSchema: {
                        type: "object",
                        properties: {},
                ...
                ...metadata.map((t) => ({
                    ...
                    annotations: {
                        ...
                        idempotentHint: ["read", "ls", "find", "grep", "write"].includes(t.name),
        ...
        server.setRequestHandler(CallToolRequestSchema, async ({ params }) => {
            try {
                ...
                if (!toolNames.includes(params.name))
            ...
            ws.on("message", (raw) => {
                ...
                try {
                    ...
                    if (!attached) {
                        ...
                        ws.send(JSON.stringify({ type: "ready", device_id: device.device_id }));
                    }
                    else {
