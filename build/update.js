...
const manifestUrl = "https://raw.githubusercontent.com/kumoSleeping/Chat2Pi/main/package.json";
const installUrl = "https://github.com/kumoSleeping/Chat2Pi/archive/refs/heads/main.tar.gz";
...
export async function checkForUpdates(signal) {
    ...
    try {
        ...
        if (remote.name !== "chat2pi")
            throw Error("版本信息来源不匹配");
        ...
        if (changed >= 0 && latest[changed] > current[changed]) {
            ...
            console.log(`Chat2Pi 有新版本：${local.version} → ${remote.version}（仅提示，不自动安装）\n先停止服务，再运行：${npm} install -g ${installUrl}`);
        }
        else {
