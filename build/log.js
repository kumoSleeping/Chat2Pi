...
const secretKey = /password|passwd|secret|token|authorization|cookie|api[-_]?key|device[-_]?key|login[-_]?key/i;
export function preview(value, limit = 180) {
    const text = String(value)
        .replace(/[\x00-\x1f\x7f-\x9f]/g, " ")
        .replace(/(Bearer\s+)\S+/gi, "$1[redacted]")
        .replace(/((?:password|passwd|secret|token|api[-_]?key|device[-_]?key|login[-_]?key)["']?\s*[:=]\s*["']?)[^\s"'&,;]+/gi, "$1[redacted]")
        .replace(/(https?:\/\/)[^\s/@]+:[^\s/@]+@/gi, "$1[redacted]@")
        .replace(/\s+/g, " ")
        .trim();
    return text.length > limit ? text.slice(0, limit - 1) + "…" : text;
}
function valuePreview(value, depth = 0) {
    ...
    return ("{" +
        entries
            .slice(0, 6)
            .map(([key, v]) => `${preview(key, 40)}: ${secretKey.test(key) ? '"[redacted]"' : valuePreview(v, depth + 1)}`)
            .join(", ") +
        (entries.length > 6 ? ", …" : "") +
        "}");
}
export function argumentPreview(args) {
    ...
    return preview(entries
        .slice(0, 8)
        .map(([key, value]) => `${preview(key, 40)}=${secretKey.test(key) ? '"[redacted]"' : valuePreview(value)}`)
        .join(" ") + (entries.length > 8 ? " …" : ""), 1000);
}
function shortPath(value, workspace) {
    if (!isAbsolute(value))
        return value;
...
export function operationSummary(name, args, workspace) {
    let detail;
    if (name === "bash") {
        ...
        detail = detail.replace(/^(?:[A-Za-z_][A-Za-z0-9_]*=(?:"[^"]*"|'[^']*'|[^\s]+)\s+)+/, "");
    }
    else {
        detail = shortPath(typeof args.path === "string" ? args.path : ".", workspace);
...
function fitLine(text, columns) {
    ...
    for (const { segment } of segmenter.segment(text)) {
        ...
        const wide = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}\p{Extended_Pictographic}\uFF01-\uFF60\uFFE0-\uFFE6]/u.test(segment);
        width += wide ? 2 : 1;
        if (width > columns - 1)
            return result + "…";
        result += segment;
...
export function formatLogLine(line) {
    const match = /^(\S+) \[(INFO|QUEUE|START|OK|WARN|ERROR)\] (.*)$/.exec(line.split("\t", 1)[0]);
    if (!match)
        return line;
