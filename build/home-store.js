...
export function loginPath(home, url, account) {
    return join(home, "accounts", `${serverTag(url)}--${identifier.parse(account)}.login.json`);
}
export function bindingPath(home, url, account, device) {
    return join(home, "bindings", `${serverTag(url)}--${identifier.parse(account)}--${identifier.parse(device)}.binding.json`);
}
...
export const bindingFiles = (home) => filesIn(home, "bindings", ".binding.json");
