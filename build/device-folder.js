...
export function prepareDeviceFolder(home) {
    const dir = join(home, "devices");
    mkdirSync(dir, { recursive: true, mode: 0o700 });
    const inputs = bindingFiles(home).filter((path) => path.startsWith(dir + "/") || path.startsWith(dir + "\\"));
    const prepared = inputs.map((path) => {
        const text = readFileSync(path, "utf8");
        const raw = JSON.parse(text);
        if (raw.login_key)
            throw Error(`Account credentials belong outside the devices folder: ${path}`);
    ...
    for (const entry of prepared) {
        ...
        if (!isDeepStrictEqual(previous.value.binding, b) ||
            previous.value.device_key !== entry.value.device_key)
            throw Error(`Conflicting device files; originals preserved: ${previous.path}, ${entry.path}`);
        ...
        if (!isDeepStrictEqual(previous.value, entry.value))
            throw Error(`Conflicting local device settings; originals preserved: ${previous.path}, ${entry.path}`);
        duplicates.push(entry);
    }
    for (const entry of prepared)
    ...
    for (const entry of selected.values()) {
        ...
        if (!isDeepStrictEqual(JSON.parse(JSON.stringify(entry.value)), entry.raw)) {
...
export async function openDeviceFolder(home) {
