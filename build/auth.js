...
export class PersonalAuth {
    ...
    codes = new Map();
    ownerKey;
    constructor(origin, stateFile, ownerKeyFile) {
        this.origin = origin;
    ...
    clientsStore = {
        getClient: (id) => Object.hasOwn(this.state.clients, id)
    ...
    mount(app) {
        ...
        app.post("/approve", express.urlencoded({ extended: false, limit: "4kb" }), (req, res) => {
            ...
            if (typeof req.body.owner_key !== "string" ||
            ...
            if (r.params.state)
                url.searchParams.set("state", r.params.state);
    ...
    async revokeToken(client, request) {
        for (const grants of [this.state.access, this.state.refresh]) {
            ...
            if (grants[key]?.clientId === client.client_id)
                delete grants[key];
        }
        this.persist();
    }
