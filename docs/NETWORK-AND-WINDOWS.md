...
当前客户端在 Windows 和 macOS 统一使用 `chat2pi start` 前台运行、轮询显示日志，Ctrl+C 经本机控制接口停止服务并断联；`chat2pi start --background` 保留后台运行。不包含开机自启。旧版单配置仍可用 `run` / `agent`。确认前台运行后，可用 Windows 任务计划程序以当前用户启动同一条命令；不要无理由提升到管理员。电脑休眠时会离线，恢复后自动重连。Windows 进程树终止仍有限制，超时不能保证所有派生进程都结束，操作也不能自动重试。
