...
工具调用默认总超时为 **300 秒（5 分钟）**，`local.timeout_seconds` 可设为 1–300 秒；已有文件里的显式设置会保留。计时从设备收到调用开始，包含排队和执行时间；排队过期会明确返回“未执行”，不会继续在后台启动。Bash 参数 `timeout` 从命令开始执行计时，可以提前结束命令，但不能延长设备总预算。修改后需要重启客户端。

`local.max_concurrent` 默认 **16**，允许同一设备连接同时执行的工具调用数；可调高到 32、64 等正的安全整数，没有 16 的硬上限，设为 1 则串行。所有工具共用此额度，超过额度才先进先出排队。已有文件未设置此字段时也默认 16。提高并发会增加进程、内存和 CPU 占用，不会自动隔离文件或共享资源；修改后重启客户端生效。
