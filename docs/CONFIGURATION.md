...
工具调用默认总超时为 **300 秒（5 分钟）**，`local.timeout_seconds` 可设为 1–300 秒；已有文件里的显式设置会保留。同一设备连接串行排队，计时从设备收到调用开始，包含排队和执行时间；排队过期会明确返回“未执行”，不会继续在后台启动。Bash 参数 `timeout` 从命令开始执行计时，可以提前结束命令，但不能延长设备总预算。修改后需要重启客户端。
