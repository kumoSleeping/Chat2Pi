...
断线自动重连，但操作不自动重放。撤销绑定或停用账号会拒绝后续访问；已经开始执行的操作不能保证回滚。轮换账号登录密钥会立即使旧 OAuth 授权失效。本地日志记录时间、设备/工具、请求短编号、工作目录、截断的参数摘要、耗时及错误原因；不记录完整工具结果，常见密钥字段会遮盖，但命令或内容摘要仍可能包含敏感信息，请勿公开日志。前台终端显示精简视图，时间使用本地时区，普通操作按终端宽度截断并按状态着色（支持 `NO_COLOR`）；错误保留原因，允许换行。`runtime/service.log` 保持纯文本，仍包含成功记录及耗时等排障信息，不受前台过滤影响。关闭 Worker observability。
