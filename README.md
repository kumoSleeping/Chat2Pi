...
`start` / `restart` 启动时并行检查 GitHub `main` 的包版本，只提示更新和安装命令，不自动安装、停止或重启服务。检查最长 3 秒，失败不阻止设备启动；不会发送账号或设备凭证。可用 `--no-update-check` 或环境变量 `CHAT2PI_NO_UPDATE_CHECK=1` 跳过。版本检查不比较提交，因此发布新版时需要递增 `package.json` 的版本。
...
同一设备连接默认允许 **16 个工具调用并行执行**，读、写、编辑和 Bash 一视同仁，不需要初始化聊天会话或创建工作区。设备配置的 `local.max_concurrent` 可设为任意正的安全整数，例如 32 或 64；16 是默认值，不是硬上限，设为 1 则恢复串行。每个调用启动独立进程，并发越高，内存和 CPU 占用可能越高。

只有超过并发额度才按到达顺序排队，不返回 `Device busy`。各调用独立超时、取消和记录日志；排队和执行共用设备超时预算（默认 300 秒）。排队过期、收到取消信号或设备断线后，尚未开始的任务不会再执行。前台每次正常调用只显示一行短时间、工具和操作摘要，成功后不再输出；只有排队、失败或连接异常时补充提示。操作行不重复账号、设备名、请求编号、默认目录、超时预算或结果字节数。额度针对设备连接，不是跨账号绑定的整机锁；系统不做文件冲突判断，同时修改同一文件或共享资源仍可能冲突。
...
断线自动重连，但操作不自动重放。新版云端与客户端同时启用后，同一运行实例的重连可以接替其旧连接，无需等待旧连接过期；不同实例或旧版客户端仍受重复连接保护。实例标识只在进程内保存，重启后不会继承。网络错误按指数退避重连，HTTP 409 约 15 秒重试，401/403 或协议拒绝约 60 秒重试；均带随机抖动。日志区分心跳超时、未收到 ready、握手 HTTP 错误和远端关闭，并记录重连次数与离线时间。撤销绑定或停用账号会拒绝后续访问；已经开始执行的操作不能保证回滚。轮换账号登录密钥会立即使旧 OAuth 授权失效。本地日志记录时间、设备/工具、请求短编号、工作目录、截断的参数摘要、耗时及错误原因；不记录完整工具结果，常见密钥字段会遮盖，但命令或内容摘要仍可能包含敏感信息，请勿公开日志。前台终端显示精简视图，时间使用本地时区，普通操作按终端宽度截断并按状态着色（支持 `NO_COLOR`）；错误保留原因，允许换行。`runtime/service.log` 保持纯文本，仍包含成功记录及耗时等排障信息，不受前台过滤影响。关闭 Worker observability。
