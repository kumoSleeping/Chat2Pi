...
只有超过并发额度才按到达顺序排队，不返回 `Device busy`。各调用独立超时、取消和记录日志；排队和执行共用设备超时预算（默认 300 秒）。排队过期、收到取消信号或设备断线后，尚未开始的任务不会再执行。日志以 `QUEUE` / `START` / `OK` / `ERROR` 展示状态，并显示等待时间。额度针对设备连接，不是跨账号绑定的整机锁；系统不做文件冲突判断，同时修改同一文件或共享资源仍可能冲突。
...
断线自动重连，但操作不自动重放。撤销绑定或停用账号会拒绝后续访问；已经开始执行的操作不能保证回滚。轮换账号登录密钥会立即使旧 OAuth 授权失效。本地日志记录时间、设备/工具、请求短编号、工作目录、截断的参数摘要、耗时及错误原因；不记录完整工具结果，常见密钥字段会遮盖，但命令或内容摘要仍可能包含敏感信息，请勿公开日志。前台终端按状态着色（支持 `NO_COLOR`），`runtime/service.log` 保持纯文本。关闭 Worker observability。
